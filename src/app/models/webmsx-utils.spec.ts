...
describe('WebMSXUtils', () => {
  it('getWebMSXParams function should return WebMSX parameters for the given game', () => {
    ...
    game.setRomA('romA');
    expect(WebMSXUtils.getWebMSXParams(game)).toEqual({ROM: 'romA', PRESETS: 'OPL4'});

    game = new Game('nameDisk', '1', 1);
    game.setDiskA('diskA');
    expect(WebMSXUtils.getWebMSXParams(game)).toEqual({DISK: 'diskA', PRESETS: 'OPL4'});

    game = new Game('nameRomAndDisk', '1', 1);
    game.setRomA('romA');
    game.setDiskA('diskA');
    expect(WebMSXUtils.getWebMSXParams(game)).toEqual({ROM: 'romA', DISK: 'diskA', PRESETS: 'OPL4'});

    game = new Game('nameDiskAndSCC', '1', 1);
    game.setDiskA('diskA');
    game.setExtensionRom('scc');
    expect(WebMSXUtils.getWebMSXParams(game)).toEqual({DISK: 'diskA', PRESETS: 'SCC,OPL4'});

    game = new Game('nameRomAndSCC+', '1', 1);
    game.setRomA('romA');
    game.setExtensionRom('scc+');
    expect(WebMSXUtils.getWebMSXParams(game)).toEqual({ROM: 'romA', PRESETS: 'SCCI,OPL4'});

    game = new Game('nameTape', '1', 1);
    game.setTape('tape');
    expect(WebMSXUtils.getWebMSXParams(game)).toEqual({TAPE: 'tape', PRESETS: 'OPL4'});

    game = new Game('nameHarddisk', '1', 1);
    game.setHarddisk('harddisk');
    expect(WebMSXUtils.getWebMSXParams(game)).toEqual({HARDDISK: 'harddisk', PRESETS: 'OPL4'});

    game = new Game('nameRomAndGFX9000', '1', 1);
    game.setRomA('romA');
    game.setConnectGFX9000(true);
    expect(WebMSXUtils.getWebMSXParams(game)).toEqual({ROM: 'romA', PRESETS: 'V9990,OPL4'});

    game = new Game('nameDiskAndMouse', '1', 1);
    game.setDiskA('diskA');
    game.setInputDevice(InputDevice.indexOf('mouse'));
    expect(WebMSXUtils.getWebMSXParams(game)).toEqual({DISK: 'diskA', MOUSE_MODE: 1, PRESETS: 'OPL4'});
