...
describe('ScreenshotFilenamePipe', () => {
  it('transform function should return screenshot full path for given game and screenshots path', () => {
    ...
    expect(pipe.transform(game1, 'path/', null)).toEqual('path/2a.png');
    ...
    expect(pipe.transform(game3, 'path/', 'colecoPath/')).toEqual('path/undefineda.png');

    const game4 = new Game('name4', '345', 50);
    game4.setColecoScreenshot('screenshot');
    expect(pipe.transform(game4, 'path/', 'colecoPath/')).toEqual('colecoPath/screenshot_a.png');
  });
});
